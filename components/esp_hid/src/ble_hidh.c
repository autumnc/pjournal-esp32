            ...
            if (esp_ble_gattc_get_all_char(gattc_if, dev->ble.conn_id, service_result[s].start_handle, service_result[s].end_handle, char_result, &ccount, 0) == ESP_OK) {
                for (uint16_t c = 0; c < ccount; c++) {
                    ...
                    if (suuid == 0x1800) {
                        ...
                        // 放宽门槛: 只支持 NOTIFY(不支持 READ) 的键盘也要记录,否则永远收不到电量
                        if (cuuid == ESP_GATT_UUID_BATTERY_LEVEL && (char_result[c].properties & (ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_NOTIFY)) != 0) {
                            dev->ble.battery_handle = chandle;
                            dev->ble.battery_readable = (char_result[c].properties & ESP_GATT_CHAR_PROP_BIT_READ) != 0;
                        } else {
                            continue;
                        }
...
static void attach_report_listeners(esp_gatt_if_t gattc_if, esp_hidh_dev_t *dev)
{
    ...
    if (dev->ble.battery_handle) {
        ...
        // 主动读取一次当前电量: 很多键盘只在电量变化时推送, 连接时不会主动上报。
        // 手机正是在连接时 READ 一次才能显示电量的。
        if (dev->ble.battery_readable) {
            esp_ble_gattc_read_char(gattc_if, dev->ble.conn_id, dev->ble.battery_handle, ESP_GATT_AUTH_REQ_NO_MITM);
        }
    }
...
void esp_hidh_gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_READ_DESCR_EVT: {
        ...
        // 连接时主动读取的电池电量: 直接转发为电池事件, 不进入同步读取缓冲
        if (dev->ble.battery_handle && p_data->read.handle == dev->ble.battery_handle) {
            if (p_data->read.status == 0 && p_data->read.value_len > 0) {
                if (event_loop_handle) {
                    esp_hidh_event_data_t p = {0};
                    p.battery.dev = dev;
                    p.battery.level = p_data->read.value[0];
...
esp_hidh_dev_t *esp_ble_hidh_dev_open(esp_bd_addr_t bda, esp_ble_addr_type_t address_type)
{
    ...
    esp_ble_conn_params_t phy_1m = {
        .scan_interval = 0x0060,
        .scan_window = 0x0030,
        .interval_min = 0x0006,
        .interval_max = 0x0018,
        .latency = 0,
        .supervision_timeout = 0x012C,
        .min_ce_len = 0,
        .max_ce_len = 0,
    };
    creat_conn_params.phy_1m_conn_params = &phy_1m;
