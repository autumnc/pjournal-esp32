...
def main():
    ...
    require_source_contains("delete mode status label", UI_HELPERS_CPP.read_text(encoding="utf-8"), "return \"[删]\"")
    ...
    require_source_contains("auto phrase denoise", ime_cpp, "isWeakAutoPhraseChar")
    require_source_contains("recent delete learning guard", ime_cpp, "recentlyDeletedWord(word)")
    require_source_contains("prediction ranked candidates", ime_cpp, "RankedPredictCandidate")
    require_source_contains("prediction per-candidate key", ime_h, "_predictCandidateKeys")
    require_source_contains("prediction reject hotkey", ime_cpp, "已降低联想:")
    require_source_contains("prediction reject dict", ime_cpp, "USERPREDICT_REJECT_PATH")
    require_source_contains("prediction reject filter", ime_cpp, "rejectedPredictWord(keyText")
    require_source_contains("prediction reject context", ime_cpp, "rejectPredictWord(rejectKey, word)")
    require_source_contains("ime mode label", ime_h, "modeLabel")
    require_source_contains("ime mode label delete", ime_cpp, "return \"[删]\"")
    require_source_contains("ime mode label liangfen", ime_cpp, "return \"[两]\"")
    require_source_contains("ime mode label predict", ime_cpp, "return \"[联]\"")
    if "return \"[中]\"" in ime_cpp or "return \"[英]\"" in ime_cpp or "return \"[繁]\"" in ime_cpp:
        raise SystemExit("ime mode label: candidate strip should only show special modes")
    require_source_contains("dynamic candidate width helper", UI_HELPERS_CPP.read_text(encoding="utf-8"), "imeCandidateLineWidth")
    require_source_contains("fullscreen ime lower anchor", ui_helpers_cpp, "2 * FONT_H + 8")
    require_source_contains("candidate middle truncation", ui_helpers_cpp, "fitTextWidthMiddle")
    require_source_contains("dynamic candidate page cap", ime_cpp, "pageCount >= 9")
    require_source_contains("dynamic display width", main_cpp, "imeCandidateLineWidth()")
    ...
    editor_text = (ROOT / "main" / "screen_editor.cpp").read_text(encoding="utf-8")
    inspiration_text = (ROOT / "main" / "screen_inspiration.cpp").read_text(encoding="utf-8")
    app_h = (ROOT / "main" / "pjournal_app.h").read_text(encoding="utf-8")
    require_source_contains("editor title override context", app_h, "editorTitle")
    require_source_contains("editor title override status", editor_text, "editorStatusTitle")
    require_source_contains("inspiration editor title", inspiration_text, "ctx.editorTitle = \"灵感\"")
    require_source_contains("inspiration stashes editor session", inspiration_text, "app_editor_stash_session")
    require_source_contains("inspiration restores editor session", inspiration_text, "app_editor_restore_stashed_session")
    ...
    require_source_contains("dict bulk import api", ime_h, "addUserDictEntries")
    require_source_contains("dict bulk import use", (ROOT / "main" / "screen_settings.cpp").read_text(encoding="utf-8"), "addUserDictEntries")
    ...
    require_source_contains("ime ranking cases", (ROOT / "scripts" / "check_ime_cases.py").read_text(encoding="utf-8"), "Check approximate IME candidate ranking cases")
    require_source_contains("ime ranking case data", (ROOT / "scripts" / "ime_cases.txt").read_text(encoding="utf-8"), "shurufa\t输入法\t1")
